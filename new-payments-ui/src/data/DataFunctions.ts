...
export const getAllPaymentsRESTVersion : () => PaymentType[] = () => {
    const responsePromise: Promise<Response> = fetch('https://paymentsdemo.neueda.com/api/payment', 
        {method: 'GET', headers: {accept:'application/json'}})

    responsePromise
    .then((response) => {
        // console.log('From REST API...', response.json)
        return response.json()
    }).then((data: PaymentType[]) => {
        // console.log(data) 
        return data
    }).catch((error) => {
